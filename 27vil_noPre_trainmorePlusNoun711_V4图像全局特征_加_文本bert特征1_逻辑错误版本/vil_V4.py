...
class CatDeffTensor(nn.Module):
    def __init__(self,cat_size,out_size):
        ...
        self.dense=nn.Linear(cat_size,out_size)
        ###可以加一个全连接层
        self.dropout=nn.Dropout(0.1)
        self.sig=nn.Sigmoid()
    ...
    def forward(self,input1,input2):
        ...
        img_cat_text=self.dense(img_cat_text)
...
class IMGFilterText(nn.Module):
    ...
    def forward(self,img,text):
        ...
        img_cat_text_relu=self.relu2(img_cat_text)
...
class BlockImgText(nn.Module):
    ...
    def forward(self,img,text):
        ...
        text_cat_output=self.text_CatDeffTensor(text,img)##bug，原来使用的是text
...
class MutilWord(nn.Module):
    ...
    def forward(self,img,input_tensor2,attention_mask2):
        ...
        return vil_img_mean_k,vil_img_mean_v,img_output_mean,vil_text_mean_k,vil_text_mean_v,text_output_
...
class BertBiAttention(nn.Module):
    ...
    def forward(
        ...
        #attentionLSTM
        vil_img_mean_k,vil_img_mean_v,img_output_mean,vil_text_mean_k,vil_text_mean_v,text_output_=self.atten_img_word(img,input_tensor2,attention_mask2) 
...
@registry.register_model("vil_v4")
class ViLBERTV4(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.cuda_device=torch.device('cuda:0')
    @classmethod
    def config_path(cls):
        return "configs/models/vilbert/vil_V4_model.yaml"
