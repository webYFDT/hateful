...
class HatefulMemesFeaturesDataset(MMFDataset):
    ...
    def RandomErasing(self,data_tensor,p=0.5,p_object=0.7,sl=0.02,sh=0.4):
        ...
        return data_tensor
    #随机抹除一些obj
    def RandomErasingObj0(self,data_tensor,p=0.5,p_object=0.2,sl=0.02,sh=0.4):
        if random.uniform(0,1)>p:
            return data_tensor
        #feature_len=data_tensor.shape[1]
        for id_object in range(data_tensor.shape[0]):
    ...
    def __getitem__(self, idx):
        ...
        #随机抹除一些obj
        current_sample.image_feature_0=self.RandomErasingObj0(current_sample.image_feature_0)
        #添加图片信息
