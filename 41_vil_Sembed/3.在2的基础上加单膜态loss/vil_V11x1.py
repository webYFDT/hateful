...
class ViLBERTForClassification(nn.Module):
    def __init__(self, config):
        ...
        ###t和v再分别做loss
        self.dropout_t=nn.Dropout(self.config.hidden_dropout_prob)
        self.dropout_v=nn.Dropout(self.config.hidden_dropout_prob)
        self.classifier_t=nn.Linear(classifier_config.hidden_size, self.num_labels)
        self.classifier_v=nn.Linear(classifier_config.hidden_size, self.num_labels)
    ...
    def forward(
        ...
        ###分别求t和v的loss
        pooled_output_tt=self.dropout_t(pooled_output_t)
        pooled_output_vv=self.dropout_v(pooled_output_v)
        
        logits_t = self.classifier_t(pooled_output_tt)
        reshaped_logits_t = logits_t.contiguous().view(-1, self.num_labels)
        output["scores_t"] = reshaped_logits_t 

        logits_v = self.classifier_v(pooled_output_vv)
        reshaped_logits_v = logits_v.contiguous().view(-1, self.num_labels)
        output["scores_v"] = reshaped_logits_v          
...
@registry.register_model("vil_v11x1")
class ViLBERTV11x1(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.cuda_device=torch.device('cuda:0')
    @classmethod
    def config_path(cls):
        return "configs/models/vilbert/vil_V11x1_model.yaml"
