...
@registry.register_loss("cross_entropy_tv")
class CrossEntropyLosstv(nn.Module):
    def __init__(self, params=None):
        super().__init__()
        if params is None:
            params = {}
        self.loss_fn = nn.CrossEntropyLoss(**params)
        self.loss_fn_t = nn.CrossEntropyLoss(**params)
        self.loss_fn_v = nn.CrossEntropyLoss(**params)
    def forward(self, sample_list, model_output):
        loss_tv=self.loss_fn(model_output["scores"], sample_list.targets)
        loss_t =self.loss_fn_t(model_output["scores_t"], sample_list.targets)
        loss_v =self.loss_fn_t(model_output["scores_v"], sample_list.targets)
        
        
        return 0.5*loss_tv+0.3*loss_t+0.2*loss_v
