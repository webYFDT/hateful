...
class BertEncoder_vil(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        layer = BertLayer_vil(config)  #BertLayer替换为BertLayer_vil
...
class ViLBERTBase(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder = BertEncoder_vil(config)########和原始pytorch的BertEncoder冲突，修改为BertEncoder_vil
    ...
    def forward(
        ...
        #warnings.warn("output_all_encoded_layers+:"+str(output_all_encoded_layers))
...
class ViLBERT(BaseModel):
    ...
    def build(self):
        if self.config.training_head_type == "pretraining":
            ...
            pre_vil=torch.load('/root/.cache/torch/mmf/data/models/vilbert.pretrained.cc.original/model.pth')
...
@registry.register_model("vil_vis_cat_lessFC_shuffleData")
class VilVisCatLessFCshuffleData(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        warnings.warn('load_vil_vis_pretrain_LessFC_shuffle_data--------------'*100)
        self.build()
        
    @classmethod
    def config_path(cls):
        return "configs/models/vil_vis_cat_Pre_lessFC_noBalance_shuffleData/vvc_model_config_lessFC_shuffleData.yaml"   
    def build(self):
    ...
    def forward(self, sample_list):
        ...
        vis=self.vis.forward(sample_list)
