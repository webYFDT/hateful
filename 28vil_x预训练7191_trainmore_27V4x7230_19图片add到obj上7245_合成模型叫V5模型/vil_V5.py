...
class OutPut(nn.Module):
    def __init__(self,in_size,out_size):
        ...
        self.LayerNorm=nn.LayerNorm(out_size,eps=1e-12)
...
class CatDeffTensor(nn.Module):
    def __init__(self,cat_size,out_size):
        super(CatDeffTensor,self).__init__()
        self.dense1=nn.Linear(cat_size,cat_size//2)
        self.relu=nn.ReLU()
        self.dropout=nn.Dropout(0.1)
        self.dense2=nn.Linear(cat_size//2,out_size)
        ###可以加一个全连接层
    ...
    def forward(self,input1,input2):
        ...
        img_cat_text=torch.cat([input1,input2],1)
        
        
        img_cat_text=self.dense1(img_cat_text)
        img_cat_text=self.relu(img_cat_text)
        img_cat_text=self.dropout(img_cat_text)
        img_cat_text=self.dense2(img_cat_text)
        #img_cat_text=self.dropout(img_cat_text)
        img_cat_text_sig=self.sig(img_cat_text)
        #print(img_cat_text_sig)
        
        img_cat_text_out=input1*img_cat_text_sig
        
        return img_cat_text_out
    
class IMGFilterText(nn.Module):
    def __init__(self,cat_size, out_size):
        super(IMGFilterText,self).__init__()
        self.dense1=nn.Linear(cat_size, cat_size//2)
        self.relu1=nn.ReLU()     ######可以改
    ...
    def forward(self,img,text):
        img_cat_text=torch.cat([img,text],1)
        img_cat_text=self.dense1(img_cat_text)
        img_cat_text=self.relu1(img_cat_text)
        img_cat_text=self.dropout1(img_cat_text)
        
        img_cat_text=self.dense2(img_cat_text)
        img_cat_text_relu=self.sig2(img_cat_text)
        #img_cat_text=self.dropout2(img_cat_text)
        
        text_out=img_cat_text_relu*text
        
        return text_out

        
class BlockImgText(nn.Module):
    def __init__(self,config):
        
        super(BlockImgText,self).__init__()
        self.img_filter_text=IMGFilterText(config.global_img_att_size+config.hidden_size,1)
        
        self.img_CatDeffTensor=CatDeffTensor(config.global_img_att_size+config.hidden_size,config.v_hidden_size)
        self.text_CatDeffTensor=CatDeffTensor(config.global_img_att_size+config.hidden_size,config.hidden_size)
        
        self.img_output=OutPut(config.global_img_att_size,config.global_img_att_size)
        self.text_output=OutPut(config.hidden_size,config.hidden_size)
        
        self.dense_out_img_k=nn.Linear(config.v_hidden_size,config.global_img_att_size)
        self.dense_out_img_v=nn.Linear(config.v_hidden_size,config.global_img_att_size)
        self.dense_out_text_k=nn.Linear(config.hidden_size,config.global_img_att_size)
        self.dense_out_text_v=nn.Linear(config.hidden_size,config.global_img_att_size)
        self.dropout_img=nn.Dropout(0.1)
        self.dropout_text=nn.Dropout(0.1)
        
    def forward(self,img,text):
        text_out=self.img_filter_text(img,text)
        img_cat_output=self.img_CatDeffTensor(img,text_out)
        
        vil_img_k=self.dense_out_img_k(img_cat_output)
        vil_img_k=self.dropout_img(vil_img_k)
        
        vil_img_v=self.dense_out_img_v(img_cat_output)
        vil_img_v=self.dropout_img(vil_img_v)
        
        img_output=self.img_output(img_cat_output,img)
        
        
        
        
        text_cat_output=self.text_CatDeffTensor(text_out,img)##bug，原来使用的是text
        
        vil_text_k=self.dense_out_text_k(text_cat_output)
        vil_text_k=self.dropout_text(vil_text_k)

        vil_text_v=self.dense_out_text_v(text_cat_output)
        vil_text_v=self.dropout_text(vil_text_v)
        
        text_output=self.text_output(text_cat_output,text)
        
        return vil_img_k,vil_img_v,img_output,vil_text_k,vil_text_v,text_output
        
#使用pack处理
class MutilWord(nn.Module):
    def __init__(self,config):
        super(MutilWord,self).__init__()
        self.block=BlockImgText(config)
        self.pad_word_len=config.num_negative
    
    def forward(self,img,input_tensor2,attention_mask2):
        texts=input_tensor2
        lengths=(attention_mask2==0).sum(-1).reshape(-1)
        
        imgs=img.unsqueeze(1)
        #重复张量
        imgs_r=imgs.repeat(1,self.pad_word_len,1)
        
        #imgs_r和texts打包
        
        texts_packed=nn.utils.rnn.pack_padded_sequence(texts,lengths,batch_first=True,enforce_sorted=False)
        imgs_packed=nn.utils.rnn.pack_padded_sequence(imgs_r,lengths,batch_first=True,enforce_sorted=False)
        #送入模型
        vil_img_k,vil_img_v,img_output,vil_text_k,vil_text_v,text_output=self.block(imgs_packed.data,texts_packed.data)
        
        #打包输出
        vil_img_pack_k=torch.nn.utils.rnn.PackedSequence( data=vil_img_k,
                                                        batch_sizes=imgs_packed.batch_sizes,
                                                        sorted_indices=imgs_packed.sorted_indices,
                                                        unsorted_indices=imgs_packed.unsorted_indices)
        vil_img_pack_v=torch.nn.utils.rnn.PackedSequence( data=vil_img_v,
                                                        batch_sizes=imgs_packed.batch_sizes,
                                                        sorted_indices=imgs_packed.sorted_indices,
                                                        unsorted_indices=imgs_packed.unsorted_indices)
        
        img_output_pack=torch.nn.utils.rnn.PackedSequence( data=img_output,
                                                        batch_sizes=imgs_packed.batch_sizes,
                                                        sorted_indices=imgs_packed.sorted_indices,
                                                        unsorted_indices=imgs_packed.unsorted_indices)        

        vil_text_pack_k=torch.nn.utils.rnn.PackedSequence( data=vil_text_k,
                                                        batch_sizes=texts_packed.batch_sizes,
                                                        sorted_indices=texts_packed.sorted_indices,
                                                        unsorted_indices=texts_packed.unsorted_indices)
        vil_text_pack_v=torch.nn.utils.rnn.PackedSequence( data=vil_text_v,
                                                        batch_sizes=texts_packed.batch_sizes,
                                                        sorted_indices=texts_packed.sorted_indices,
                                                        unsorted_indices=texts_packed.unsorted_indices)
        
        text_output_pack=torch.nn.utils.rnn.PackedSequence( data=text_output,
                                                        batch_sizes=texts_packed.batch_sizes,
                                                        sorted_indices=texts_packed.sorted_indices,
                                                        unsorted_indices=texts_packed.unsorted_indices)  
        
        #解压打包
        vil_img_k_,_=nn.utils.rnn.pad_packed_sequence(vil_img_pack_k,batch_first=True,total_length=self.pad_word_len)
        vil_img_v_,_=nn.utils.rnn.pad_packed_sequence(vil_img_pack_v,batch_first=True,total_length=self.pad_word_len)
        img_output_,_=nn.utils.rnn.pad_packed_sequence(img_output_pack,batch_first=True,total_length=self.pad_word_len)
        
        vil_text_k_,_=nn.utils.rnn.pad_packed_sequence(vil_text_pack_k,batch_first=True,total_length=self.pad_word_len)
        vil_text_v_,_=nn.utils.rnn.pad_packed_sequence(vil_text_pack_v,batch_first=True,total_length=self.pad_word_len)
        text_output_,_=nn.utils.rnn.pad_packed_sequence(text_output_pack,batch_first=True,total_length=self.pad_word_len)
        
        
        #均值
        vil_img_mean_all_k=vil_img_k_.mean(1)
        vil_img_mean_all_v=vil_img_v_.mean(1)
        img_output_all=img_output_.mean(1)
        vil_text_all_k=vil_text_k_.mean(1)
        vil_text_all_v=vil_text_v_.mean(1)
        text_output_all=text_output_.mean(1)
        
        lengths_all=lengths.unsqueeze(1)
        
        lengths_mean=lengths_all/float(self.pad_word_len)
        
        vil_img_mean_k=vil_img_mean_all_k/lengths_mean
        vil_img_mean_v=vil_img_mean_all_v/lengths_mean
        img_output_mean=img_output_all/lengths_mean
        
        vil_text_mean_k=vil_text_all_k/lengths_mean
        vil_text_mean_v=vil_text_all_v/lengths_mean
        text_output_mean=text_output_all/lengths_mean
        
        
        '''       
        print(vil_img_.shape)
        print(img_output_.shape)
        print(vil_text_.shape)
        print(text_output_.shape)
        '''
        
        return vil_img_mean_k,vil_img_mean_v,img_output_mean,vil_text_mean_k,vil_text_mean_v
...
class BertBiAttention(nn.Module):
    def __init__(self, config):
        ...
        self.atten_img_word=MutilWord(config)
    ...
    def forward(
        ...
        #attentionLSTM
        vil_img_mean_k,vil_img_mean_v,img_output_mean,vil_text_mean_k,vil_text_mean_v=self.atten_img_word(img,input_tensor2,attention_mask2) 
        
        # for vision input.
        
        mixed_query_layer1 = self.query1(input_tensor1)
        
        #warnings.warn(str(mixed_query_layer1.shape))        #[64, 100, 1024]
        
        mixed_key_layer10 = self.key1(input_tensor1)
        #warnings.warn(str(mixed_key_layer1.shape))
        #warnings.warn(str(atten_vec.shape))
        mixed_key_layer1=torch.cat([vil_img_mean_k.unsqueeze(1),mixed_key_layer10],1)
        #warnings.warn(str(mixed_key_layer1.shape))             # [64, 101, 1024]     
        mixed_value_layer10 = self.value1(input_tensor1)
        mixed_value_layer1=torch.cat([vil_img_mean_v.unsqueeze(1),mixed_value_layer10],1)
        #warnings.warn(str(mixed_value_layer1.shape))           # [64, 101, 1024]
        # mixed_logit_layer1 = self.logit1(input_tensor1)

        query_layer1 = self.transpose_for_scores(mixed_query_layer1)
        #warnings.warn(str(query_layer1.shape))                  #[64, 8, 100, 128]
        key_layer1 = self.transpose_for_scores(mixed_key_layer1)
        #warnings.warn(str(key_layer1.shape))                    #[64, 8, 101, 128]
        value_layer1 = self.transpose_for_scores(mixed_value_layer1)
        #warnings.warn(str(value_layer1.shape))                  # [64, 8, 101, 128]
        # logit_layer1 = self.transpose_for_logits(mixed_logit_layer1)

        # for text input:
        mixed_query_layer2 = self.query2(input_tensor2)
        #warnings.warn(str(mixed_query_layer2.shape))            # [64, 128, 1024]
        mixed_key_layer20 = self.key2(input_tensor2)
        #warnings.warn(str(mixed_key_layer2.shape))
        mixed_key_layer2=torch.cat([vil_text_mean_k.unsqueeze(1),mixed_key_layer20],1)
        #warnings.warn(str(mixed_key_layer2.shape))              # [64, 129, 1024]
        mixed_value_layer20 = self.value2(input_tensor2)
        mixed_value_layer2=torch.cat([vil_text_mean_v.unsqueeze(1),mixed_value_layer20],1)
        #warnings.warn(str(mixed_value_layer2.shape))            # ([64, 129, 1024]
        ...
        '''
        warnings.warn(str(query_layer2.shape))                      #[64, 8, 128, 128]
        warnings.warn(str(key_layer2.shape))                        #[64, 8, 129, 128]
        warnings.warn(str(value_layer2.shape))                      #[64, 8, 129, 128]
        '''
        ...
        '''
        warnings.warn(str(attention_scores1.shape))                 #[64, 8, 128, 101]
        warnings.warn(str(attention_mask1.shape))                   #[64, 1, 1, 100]
        warnings.warn(str(attention_mask1))
        
        with open('attention_mask1','wb') as f:
            pk.dump(attention_mask1,f)
        '''
        temp= attention_mask1[:,0,0,0]
        temp=temp.unsqueeze(1).unsqueeze(2).unsqueeze(3)
        attention_mask1=torch.cat([temp,attention_mask1],3)
        ...
        '''
        with open('attention_mask2.pkl','wb') as f:
            pk.dump(attention_mask2,f)
        '''
        temp2= attention_mask2[:,0,0,0]
        temp2=temp2.unsqueeze(1).unsqueeze(2).unsqueeze(3)
        attention_mask2=torch.cat([temp2,attention_mask2],3)  
        '''
        warnings.warn(str(attention_scores2.shape))                 # [64, 8, 100, 129]
        warnings.warn(str(attention_mask2.shape))                   #[64, 1, 1, 102]
        '''
        ...
        return context_layer1, context_layer2, attn_data,img_output_mean
...
class BertConnectionLayer(nn.Module):
    def __init__(self, config):
        ...
        self.biOutput = BertBiOutput(config) #(FC)ADD & Norm 1

        self.v_intermediate = BertImageIntermediate(config) #FC & 激活函数
        self.v_output = BertImageOutput(config)   #ADD & Norm 2

        self.t_intermediate = BertIntermediate(config) #FC & 激活函数
        self.t_output = BertOutput(config) #ADD & Norm 2
    ...
    def forward(
        ...
        bi_output1, bi_output2, co_attention_probs,img_output_mean = self.biattention(
            input_tensor1,
            attention_mask1,
            input_tensor2,
            attention_mask2,
        ...
        return layer_output1, layer_output2, co_attention_probs,img_output_mean
    ...
    def forward(
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            if self.with_coattention:
                # do the bi attention.
                image_embedding, txt_embedding, co_attention_probs,img = self.c_layer[
                    count
                ](
                    image_embedding,
                    image_attention_mask,
        ...
        #warnings.warn(str(output_dict))
        g = make_dot(image_embedding)        
        g.render('vil_V3', view=False)
        os._exit()
        '''
...
class ViLBERTBase(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.init_img_h_c=nn.Linear(config.global_img_size, config.global_img_att_size)
    ...
    def forward(
        ...
        img=self.init_img_h_c(img1)
        #img_h_c=(torch.cat([img_h_c.unsqueeze(0),img_h_c.unsqueeze(0)],0),torch.cat([img_h_c.unsqueeze(0),img_h_c.unsqueeze(0)],0))
...
class ViLBERTForClassification(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        bert_path='/root/.cache/torch/mmf/data/models/visual_bert.finetuned.hateful_memes.direct/model.pth'
        '''
        self.bert = ViLBERTBase.from_pretrained(
            self.config.bert_model_name,
            config=BertConfig.from_dict(
...
@registry.register_model("vil_v5")
class ViLBERTV5(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.cuda_device=torch.device('cuda:0')
    @classmethod
    def config_path(cls):
        return "configs/models/vilbert/vil_V5_model.yaml"
    ...
    def get_image_and_text_features(self, sample_list):
        ...
        if sample_list.dataset_name == "nlvr2":
            ...
            #image_location 5维度向量分别代表box坐标4个值+box面积相对于原始图像面积的比例
            image_location[:, :, 0] = image_location[:, :, 0] / image_w[:, None]
            image_location[:, :, 1] = image_location[:, :, 1] / image_h[:, None]
            image_location[:, :, 2] = image_location[:, :, 2] / image_w[:, None]
            image_location[:, :, 3] = image_location[:, :, 3] / image_h[:, None]
    ...
    def forward(self, sample_list):
        '''
        import pickle as pk
        with open('sample_list','wb') as f:
            pk.dump(sample_list,f)
        os._exit()
        '''
        #warnings.warn(str(sample_list.keys()))
        ...
        params['img']=sample_list.img
        params['img']=params['img'].to(self.cuda_device)
        '''
        warnings.warn(str(params['img'].shape))
        warnings.warn(str(params['text'].shape))
        warnings.warn(str(params['word_len'].shape))
        warnings.warn(str(params['word_len']))
        '''
        '''
        params["input_ids"], params["token_type_ids"],params["attention_mask"]都是本文件之前处理得到
        bert_input_ids = sample_list.input_ids
        bert_input_mask = sample_list.input_mask
        bert_input_type_ids = sample_list.segment_ids
        '''
