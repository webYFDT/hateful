...
class ViLBERT(BaseModel):
    ...
    def build(self):
        if self.config.training_head_type == "pretraining":
            ...
            pre_vil=torch.load('/mnt1/liu1/41_vil_Sembed/save/vil_v11_final.pth')
...
class VisualBERT(BaseModel):
    ...
    def build(self):
        if self.config.training_head_type == "pretraining":
            ...
            """
            pre_vis=torch.load('/mnt1/liu1/50.1_vis_Sembed_Pre/save/vis_V11_final.pth')
            '''
...
@registry.register_model('vil_vis_catv621')
class VilVisCatV621(BaseModel):
    def __init__(self,config):
        super().__init__(config)
        self.config=config
        #self.build()
    ...
    def config_path(cls):
        #填写配置文件的绝对我、路径
        return "configs/models/vil_vis_cat_Pre_lessFC_noBalance_shuffleData/vil_vis_catV621_model.yaml"
